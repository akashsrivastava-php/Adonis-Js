...
class PostController {
  ...
  async index({view, auth, response}){
    const postList = await Post.query().with('user').with('comments', (builder)=>builder.with('user')).where('user_id', auth.user.id).paginate(1,postPerPage)
    //response.send(postList.toJSON())
  ...
  async postPagination({params, response, auth}){
    const { page } = params
    const postData = await Post.query().with('user').with('comments', (builder)=>builder.with('user')).where('user_id', auth.user.id).paginate(page,postPerPage)
    let content = ''
